...
function TeacherView() {
    ...
    return (
      <>
        ...
        {submission.map((sub, index) => (
          <a
            key={index}
            href={`http://localhost:3001/assets/${
  ...
  const fetchStudentSubmissions = async (assignmentId) => {
    try {
      const response = await axios.get(
        `http://localhost:3001/api/assignment/${assignmentId}`
      );
  ...
  const fetchStudentDetails = async (studentId) => {
    try {
      const response = await axios.get(
        `http://localhost:3001/auth/students/${studentId}`
      );
                ...
                <div className="col-md-9">
                  <div>
                    <h3 className="card-title">
                      <a
                        href={`http://localhost:3001/${assignment.picturePath}`}
                        target="_blank"
                      >
                        {assignment.title}
                      </a>
